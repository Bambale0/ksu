...
**Status:** shell/navigation contract introduced on 2026-08-12.
...
The first shell version intentionally does not invent a second payment implementation. It shows authoritative wallet data from:
...
Payment package/provider checkout UX belongs to the next Wallet/Payments product epic. The shell already provides the stable Wallet navigation target for it.
...
The backend continues to validate the Telegram signature. Model catalog/quote remain public according to the existing API contract; wallet/history/profile are authenticated.
...
The shell keeps semantic headings and named navigation, uses actual buttons/links for interactions, preserves focus indication and focuses the new view heading after navigation. `prefers-reduced-motion` disables transition/animation behavior.

## Files

```text
app/web/mini_app/index.html             semantic product shell + existing generator mount
...
Python contract tests additionally assert the stable shell DOM IDs, top-level tabs, Telegram safe-area/back/theme integration and absence of `initDataUnsafe` as an auth source in shell code.

## Follow-up feature boundaries

The shell deliberately establishes navigation before filling every tab with every business workflow. Remaining feature epics should extend the existing targets rather than adding new floating entrypoints:

1. Wallet/Payments checkout inside Wallet.
2. Partner cabinet/withdrawals inside Profile.
3. Support/notifications/profile management inside Profile/More.
4. Likes/subscriptions where content discovery/history surfaces need them.
5. Visual admin remains outside the user Mini App.
