...
| --- | --- | --- | --- | --- | --- |
| ADV-001 | High | Private repeat | The earlier repeat flow hid the source prompt in UI but still transported the recreate recipe to the browser, exposing prompt/settings through network/devtools. | Remediated in PR #387; CI pending | Backend + Playwright regression: hidden recipe never appears in bootstrap, DOM or request bodies. |
| ADV-002 | High | Private repeat / media ownership | Private-repeat accepted any syntactically URL-like HTTPS value as a “recipient-owned” reference. A caller could bypass the UI and inject another user’s or attacker-controlled URL. | Remediated in PR #387; CI pending | Quote/launch now require a ready `UserReference` owned by the authenticated user, created by `mini_app_upload`, and using product-owned reference storage. |
| ADV-003 | High | Private repeat / information disclosure | Private-repeat `/quote` delegated the full normal quote response, including `billing_seconds`, leaking a hidden source setting. | Remediated in PR #387; CI pending | Private quote is allow-listed to payable `cost_rox`; regression asserts hidden quote metadata is absent. |
| ADV-004 | Medium | Private repeat / legacy compatibility | Legacy generations whose source reference lives in top-level `input_url` could not be satisfied by the repeat UI because `input_url` is not a model UI field. | Remediated in PR #387; CI pending | Playwright covers a synthetic recipient upload for `reference_fields: ["input_url"]`. |
| ADV-005 | Medium | Private repeat / validation | `references_required=true` with no explicit `reference_fields` did not force the backend to receive a reference; an empty payload could reach model validation/provider logic. | Remediated in PR #387; CI pending | Server-side merge rejects missing required recipient media before quote/launch. |
...
| ADV-007 | High | KIE webhook authentication | KIE callback URLs include the long-lived shared `KIE_WEBHOOK_HMAC_KEY` as `?token=...`. Query strings are routinely retained in provider/proxy/access logs, turning the HMAC key into a bearer credential. The same callback also carries a client-visible `generation_id` binding hint. | Confirmed; remediation next | Remove the shared secret from callback URLs. Require KIE's documented timestamp/signature headers and separately authenticate any generation binding hint. |
...
| Provider callbacks | authenticity, task ownership, replay, unexpected payloads, stuck states | In progress; ADV-007 confirmed |
...
KIE's current webhook security contract signs `taskId + "." + timestamp` with HMAC-SHA256 and sends `X-Webhook-Timestamp` / `X-Webhook-Signature`. The shared HMAC key is meant to stay secret and does not need to be placed in the callback URL. The existing query-token fallback therefore weakens the documented design and is tracked as ADV-007.
