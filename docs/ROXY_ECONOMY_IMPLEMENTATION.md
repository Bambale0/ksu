...
If `ROX_PACKAGES_JSON` or `CARD_PACKAGES_JSON` are explicitly configured, amounts are expressed in public ROX. Every RUB package must declare `amount`, `credits` and `bonus_credits` explicitly; implicit derivation and implicit bonus matrices are rejected at startup. The explicit provider price means customers receive the configured ROX amount and pay the configured RUB amount. Use that form when mirroring independently priced provider catalogs such as Lava Top.
