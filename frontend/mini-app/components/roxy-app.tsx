...
export function RoxyApp() {
  ...
  const [previewSurface, setPreviewSurface] = useState<"private" | "feed">("private");
  ...
  return (
    <div className="roxy-app">
      ...
      {preview && <Preview item={preview} surface={previewSurface} onClose={() => setPreview(null)} onPublished={async () => { await loadProfile(); showToast("Работа опубликована в профиле"); }} showToast={showToast} />}
...
function HomeScreen({ models, recent, onNavigate, onCreate, onPreview }: { models: GenerationModel[]; recent: Generation[]; onNavigate: (route: Route) => void; onCreate: (media: "image" | "video" | "audio") => void; onPreview: (item: Generation) => void }) {
  const counts = useMemo(() => ({
    ...
    audio: models.filter((m) => m.media_type === "audio").length,
  }), [models]);
  return (
...
function CreateScreen({ models, families, me, onBalance, onCreated, showToast }: { models: GenerationModel[]; families: GenerationModelFamily[]; me: Me | null; onBalance: () => Promise<Me>; onCreated: (items: Generation[]) => void; showToast: (message: string) => void }) {
  ...
  const updateValue = (name: string, value: unknown) => {
    ...
    persist(selected.id, { ...draft, values: { ...draft.values, [name]: value } });
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const next = await api.quote(buildPayload(selected, draft));
        if (quoteSeq.current === seq) { setQuote(next); setQuoteError(""); }
      } catch (error) {
        if (quoteSeq.current === seq) { setQuote(null); setQuoteError(error instanceof Error ? error.message : "Не удалось рассчитать цену"); }
      }
    }, 320);
    ...
    <section className="screen create-screen">
      ...
      <div className="create-layout">
        <div className="create-controls">
          ...
          {groups.map((group) => {
            const grouped = fields.filter((field) => (field.group || "main") === group.id || (groups.length === 1 && !field.group));
            if (!grouped.length) return null;
            return <div className="panel" key={group.id}><h2>{group.title}</h2><div className="form-stack">{grouped.map((field) => <DynamicField key={field.name} field={field} value={draft.values[field.name]} onChange={(value) => updateValue(field.name, value)} onUpload={async (files) => {
              setUploading(true);
              try {
                const max = field.control === "file" ? 1 : field.max_items || 20;
                const urls: string[] = field.control === "files" && Array.isArray(draft.values[field.name]) ? [...draft.values[field.name] as string[]] : [];
                for (const file of files.slice(0, Math.max(0, max - urls.length))) {
                  if (field.max_size_mb && file.size > field.max_size_mb * 1024 * 1024) { showToast(`${file.name}: максимум ${field.max_size_mb} МБ`); continue; }
                  const uploaded = await api.upload(file);
                  if (field.control === "file") { updateValue(field.name, uploaded.url); break; }
                  urls.push(uploaded.url);
                }
                if (field.control === "files") updateValue(field.name, urls);
                notify("success");
              } catch (error) { notify("error"); showToast(error instanceof Error ? error.message : "Ошибка загрузки"); }
...
function Preview({ item, surface, onClose, onPublished, showToast }: { item: Generation | FeedCard; surface: "private" | "feed"; onClose: () => void; onPublished: () => Promise<void>; showToast: (message: string) => void }) {
  const [publishing, setPublishing] = useState(false);
  const url = mediaUrl(item);
  const type = mediaType(item);
  const publish = async () => {
    setPublishing(true);
    try { await api.publish(item.id, "profile"); notify("success"); await onPublished(); }
...
function WalletSheet({ me, onClose, onRefresh, showToast }: { me: Me | null; onClose: () => void; onRefresh: () => Promise<Me>; showToast: (message: string) => void }) {
  ...
  const pay = async () => {
    ...
    setPaying(true);
    try {
      const payment = await api.createPayment(selected);
      if (!payment.payment_url) throw new Error("Платёжная ссылка не получена");
      const tg = telegram();
      if (tg?.openLink) tg.openLink(payment.payment_url);
      else window.open(payment.payment_url, "_blank", "noopener,noreferrer");
      showToast("Платёж создан");
      window.setTimeout(() => void onRefresh(), 2000);
