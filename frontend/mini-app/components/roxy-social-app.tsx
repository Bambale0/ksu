...
function profileLink(stats: PartnerStats | null, me: Me | null): string | null {
  if (me?.profile_link) return me.profile_link;
  if (stats?.profile_link) return stats.profile_link;
  if (stats?.author_profile_link) return stats.author_profile_link;
  if (!stats?.referral_link || !me?.telegram_id) return null;
  return stats.referral_link
    .replace(`ref_${me.telegram_id}`, `profile_${me.telegram_id}_ref_${me.telegram_id}`)
    .replace("start=ref_", "start=profile_");
}
...
function PartnerScreen({ me, stats, rewards, invitations, onRefresh, showToast }: { me: Me | null; stats: PartnerStats | null; rewards: ReferralReward[]; invitations: ReferralInvitation[]; onRefresh: () => void; showToast: (message: string) => void }) {
  ...
  return <section className="screen"><ScreenHead kicker="Партнёрам" title="Кабинет автора" copy="Финансовая партнёрская программа включается только после активации вашего промокода пользователем." />
    <div className="profile-stats panel"><div><strong>{compact(stats?.promo_first_line)}</strong><span>по промокоду</span></div><div><strong>{compact(stats?.first_line_percent)}%</strong><span>с пополнений</span></div><div><strong>{compact(stats?.partner_balance_rub)} ₽</strong><span>доступно</span></div></div>
    <div className="panel"><span className="kicker">Условия программы</span><h2>{stats?.promo_program_active === false ? "Программа на паузе" : "Промокод активирует бонусы"}</h2><p className="muted">Пользователь получает +{compact(stats?.promo_welcome_rox)} ROX после активации промокода. Вы получаете {compact(stats?.first_line_percent)}% с его успешных пополнений и +{compact(stats?.promo_topup_partner_rox)} ROX за каждое успешное пополнение. За обычное приглашение начислений нет.</p>{stats?.promo_program_active === false ? <p className="muted">Новые финансовые начисления временно остановлены, но существующая промо-привязка сохраняется.</p> : null}</div>
    <div className="panel"><span className="kicker">Профиль автора</span><p className="muted">{pLink || stats?.referral_link || "Ссылка появится после входа в ROXY."}</p>{pLink && <button className="primary wide" type="button" onClick={() => void copy(pLink)}>Скопировать профиль</button>}{stats?.referral_link && <button className="secondary wide" type="button" onClick={() => void copy(stats.referral_link)}>Скопировать ссылку приглашения</button>}{stats?.partner_chat_url && <a className="secondary wide" href={stats.partner_chat_url} target="_blank" rel="noreferrer">Чат партнёров</a>}</div>
    <SectionTitle kicker="Доход" title="Последние денежные начисления" action="Обновить" onAction={onRefresh} />
    <div className="transaction-list">{rewards.length ? rewards.map((reward) => <div className="transaction" key={reward.id}><div><strong>{reward.source_user?.first_name || reward.source_user?.username || "Пополнение реферала"}</strong><small>{dateLabel(reward.created_at)} · {statusLabel(reward.status)}</small></div><span>+{compact(reward.net_amount || reward.amount)} ₽</span></div>) : <Empty text="Денежных начислений пока нет." />}</div>
    <SectionTitle kicker="Пользователи" title="Последние привязки" />
    <div className="transaction-list">{invitations.filter((item) => item.line === 1).length ? invitations.filter((item) => item.line === 1).map((item) => <div className="transaction" key={item.user_id}><div><strong>{item.first_name || item.username || "Пользователь"}</strong><small>{dateLabel(item.joined_at)} · {item.source === "promo" ? "промокод" : "ссылка · без финансовых бонусов"}</small></div><span>{item.username ? `@${item.username}` : "—"}</span></div>) : <Empty text="Привязанных пользователей пока нет." />}</div>
  </section>;
}
