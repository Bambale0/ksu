...
function DynamicField({ field, value, onChange, onUpload }: { field: UiField; value: unknown; onChange: (value: unknown) => void; onUpload: (files: File[]) => Promise<void> }) {
  ...
  if (field.control === "textarea" || field.control === "json") return <label className="field"><span className="label">{label}{field.required ? " *" : ""}</span><textarea className="control textarea" placeholder={field.placeholder || ""} maxLength={field.max_length} value={value == null ? "" : typeof value === "string" ? value : JSON.stringify(value, null, 2)} onChange={(e) => onChange(e.target.value)}/></label>;
...
function PartnerScreen({ me, stats, rewards, invitations, onRefresh, showToast }: { me: Me | null; stats: PartnerStats | null; rewards: ReferralReward[]; invitations: ReferralInvitation[]; onRefresh: () => void; showToast: (message: string) => void }) {
  ...
  const promoCodes = stats?.promo_codes || [];
  const registrationWelcome = compact(stats?.registration_bonus_rox || stats?.welcome_bonus_rox || stats?.promo_welcome_rox || 0);
  const promoTopupUser = compact(stats?.promo_topup_user_rox || 0);
  const promoTopupMin = compact(stats?.promo_topup_user_min_rub || 0);
  ...
  return <section className="screen"><ScreenHead kicker="Партнёрам" title="Кабинет автора" copy="Делитесь ссылкой, приглашайте друзей и отслеживайте бонусы в одном месте." />
    ...
    <div className="panel"><span className="kicker">Профиль автора</span><p className="muted">{pLink || stats?.referral_link || "Ссылка появится после входа в ROXY."}</p>{pLink && <button className="primary wide" type="button" onClick={() => void copy(pLink)}>Скопировать профиль</button>}{stats?.referral_link && <button className="secondary wide" type="button" onClick={() => void copy(stats.referral_link)}>Скопировать обычную реферальную ссылку</button>}<p className="muted">Обычная реферальная ссылка учитывает приглашение, но сама по себе не включает финансовые бонусы.</p>{stats?.partner_chat_url && <a className="secondary wide" href={stats.partner_chat_url} target="_blank" rel="noreferrer">Чат партнёров</a>}</div>
    <div className="panel"><span className="kicker">Промокоды</span><h2>Ваши промокоды</h2><p className="muted">Новый пользователь получает +{registrationWelcome} ROX сразу после регистрации. Промокод вводится отдельно и не выдаёт этот бонус повторно. При пополнении от {promoTopupMin} ₽ активный промокод добавляет +{promoTopupUser} ROX поверх обычного бонуса пакета. Партнёр получает свою отдельную выплату с успешного пополнения.</p>{!stats?.promo_program_active ? <p className="muted">Бонусная программа временно приостановлена. Коды и привязки сохраняются.</p> : null}{promoCodes.length ? <div className="transaction-list">{promoCodes.map((promo) => <div className="transaction" key={promo.id}><div><strong>{promo.code}</strong><small>{promo.is_active ? "активен" : "выключен"} · использовано {compact(promo.uses_count)}/{promo.max_uses == null ? "∞" : compact(promo.max_uses)}</small></div><span>—</span></div>)}</div> : <p className="muted">У вас пока нет выпущенных промокодов.</p>}</div>
