...
export function WalletParity() {
  ...
  return createPortal(
    <div className="wallet-parity-link">
      ...
      <small>ЮKassa остаётся основным способом. Подарочные ROX пакета начисляются после успешной оплаты. Активный промокод дополнительно даёт +50 ROX при подходящем пополнении от 1000 ₽. Lava Top — резерв для карты, CryptoBot — для криптовалюты.</small>
    </div>,
    host,
  );
}
