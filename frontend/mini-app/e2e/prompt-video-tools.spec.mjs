...
test('video prompt mode has no duration picker and submits gallery video without duration_seconds', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '5 сек', exact: true })).toHaveCount(0);
  await expect(page.getByRole('button', { name: '10 сек', exact: true })).toHaveCount(0);
  await expect(page.getByRole('button', { name: '15 сек', exact: true })).toHaveCount(0);
  ...
  await page.getByRole('button', { name: 'Создать prompt', exact: true }).click();
