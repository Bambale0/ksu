  ...
  await page.addInitScript((startParam) => {
    ...
    window.Telegram = {
      WebApp: {
        ...
        // Telegram intentionally keeps this value alive after internal navigation.
...
test('shared trend keeps Back through the gate and native Back returns once to Catalog', async ({ page }) => {
  ...
  // The first raw BackButton decision happened on the Main Mini App deep-link
  // gate. It must be show (Back), never hide (Telegram Close).
