...
const scenarios = Array.from({ length: 100 }, (_, index) => ({
  ...
  media: (["all", "image", "video", "audio"] as const)[index % 4],
}));
...
async function mockApi(page: Page) {
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path.endsWith("/like") && method === "POST") return json({ id: "feed-1", surface: "feed", liked_by_me: true, likes_count: 13 });
    if (path.endsWith("/like") && method === "DELETE") return json({ id: "feed-1", surface: "feed", liked_by_me: false, likes_count: 11 });
    ...
    if (path === "/api/v1/payments/card/packages") return json({ provider: "card", label: "Оплата", currencies: ["RUB"], packages: { starter: { credits: "100", prices: { RUB: "100" } } } });
    if (path === "/api/v1/payments/card/checkout") return json({ id: "pay1", status: "created", payment_url: "https://example.test/pay" });
    if (path === "/api/v1/uploads/kie") return json({ url: "/uploads/reference.png", name: "reference.png" });
...
test.describe("ROXY Mini App 100 пользовательских сценариев", () => {
  for (const scenario of scenarios) {
    test(`scenario ${scenario.id}: ${scenario.route} ${scenario.viewport.width}x${scenario.viewport.height}`, async ({ page }) => {
      ...
      await expect(body).not.toContainText(/KIE|provider|server routes|media routes|ui_schema|backend-каталог/i);
      ...
      if (scenario.route === "catalog") {
        ...
        await expect(body).not.toContainText("Модели");
      }
