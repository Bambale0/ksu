...
test('large trend preview upload scales the deadline to the file size', async ({ page }) => {
  await page.addInitScript(() => {
    const original = Object.getOwnPropertyDescriptor(Blob.prototype, 'size');
    Object.defineProperty(File.prototype, 'size', {
      configurable: true,
      get() {
        if (this.name === 'slow-reference.mp4') return 200 * 1024 * 1024;
  ...
  let uploadRoute;
  await page.route('**/api/v1/uploads/kie', (route) => { uploadRoute = route; });
  await form.locator('input[type="file"]').setInputFiles({ name: 'slow-reference.mp4', mimeType: 'video/mp4', buffer: Buffer.from('preview') });
  await expect.poll(() => Boolean(uploadRoute)).toBe(true);
