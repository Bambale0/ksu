...
async def robust_route(page: Page, name: str) -> None:
    """Prefer visible primary navigation, then use canonical deep routes."""
...
async def scenario_wallet(page: Page, report: suite.legacy.Report) -> None:
    """Exercise the wallet as the balance sheet users actually open."""
...
# v2 delegates to the legacy module's main(), so patch the globals that main
# resolves instead of only replacing names on the wrapper module.
