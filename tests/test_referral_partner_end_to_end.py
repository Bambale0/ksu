...
from app.services.partner import PartnerInsufficientFunds, PartnerService
...
async def test_partner_promo_payment_is_first_line_only_and_refunds_are_proportional(
    ...
    monkeypatch.setattr(
        settings,
        "card_packages_json",
        '{"p300":{"credits":"300","bonus_credits":"30","prices":{"RUB":"326.10"}}}',
    )
    ...
    async with SessionFactory() as session:
        ...
        session.add(promo)
        # promo_id must reference the persisted promo row, so flush first.
        await session.flush()
        session.add_all(
            [
                promo,
                ...
                # Partner promo payments require the buyer to be attached via
                # the partner's promo code, not a plain referral link.
                ReferralRelation(
                    referred_user_id=buyer.id,
        ...
        # 300 base credits + the promo-gated package bonus of 30 ROX.
        ...
        await PaymentService.apply_reversal(
            ...
            provider="card",
            idempotency_key=f"refund-half:{payment.id}",
            reason="e2e partial refund retry",
            provider_payload={"refunded": "163.05"},
        )
        assert (await PartnerService.accounting(session, first_line.id))["total_earned"] == Decimal(
            "48.91"
...
async def test_partner_wallet_transfer_replay_must_match_original_amount(
    ...
    async with SessionFactory() as session:
        ...
        wallet = await session.get(Wallet, partner.id)
        assert wallet is not None
        assert Decimal(wallet.balance) == Decimal("10.00")
...
async def test_admin_partner_analytics_include_actual_available_accounting(
    ...
    async with SessionFactory() as session:
        ...
        # Accounting totals are program-wide, so capture a baseline and assert
        # deltas instead of absolute values (safe against pre-existing data).
        baseline = {
            key: Decimal(value)
            for key, value in (
                await AdminPartnerService.analytics(session, admin=admin)
            )["accounting"].items()
        }
        ...
        deltas = {
            key: Decimal(analytics["accounting"][key]) - baseline[key]
            for key in analytics["accounting"]
        }
        assert deltas == {
            "total_earned": Decimal("30.00"),
            "available": Decimal("15.00"),
            "pending_rewards": Decimal("0"),
            "pending_withdrawals": Decimal("5.00"),
            "reserved_or_paid": Decimal("5.00"),
            "transferred_to_rox": Decimal("10.00"),
        }
    ...
    async def withdraw() -> str:
        async with SessionFactory() as session:
            try:
                ...
                await session.commit()
                return "withdrawn"
            except PartnerInsufficientFunds:
                await session.rollback()
                return "blocked"
