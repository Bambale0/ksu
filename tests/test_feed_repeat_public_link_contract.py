...
def test_public_post_link_uses_bot_handoff_even_without_short_name(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert bot_start_link(payload) == handoff
    assert mini_app_deep_link(payload) == handoff
    assert _direct_mini_app_link(handoff) == handoff
