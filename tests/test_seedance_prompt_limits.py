...
def test_seedance_prompt_limits_match_provider_contract() -> None:
...
def test_miniapp_schema_and_secondary_editors_expose_exact_limits() -> None:
    ...
    presets = Path("frontend/mini-app/app/presets/page.tsx").read_text(encoding="utf-8")
    remix = Path("frontend/mini-app/app/remix/page.tsx").read_text(encoding="utf-8")
    assert "promptMaxLength" in presets
    assert "maxLength={promptMaxLength}" in presets
    assert "draft.prompt_max_length || 8000" in remix
