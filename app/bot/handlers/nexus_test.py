...
async def nexus_test_generate(callback: CallbackQuery, state: FSMContext) -> None:
    if not _is_env_admin(callback.from_user.id):
        await state.clear()
        await callback.answer("Нет доступа", show_alert=True)
        return
    ...
    async with SessionFactory() as session:
        task = await NexusAdminTaskService.enqueue(
            ...
            idempotency_key=idempotency_key or f"ksu-nexus-test:{callback.from_user.id}:{uuid.uuid4()}",
        )
