...
async def _view(session: SessionDep, job: BatchGenerationJob, *, replayed: bool | None = None) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "initial_cost_credits": amount(job.initial_cost_rox),
        "total_charged_credits": amount(job.total_charged_rox),
        "items": [
            {
                "id": str(item.id),
                ...
                "generation": {
                    ...
                    "cost_credits": amount(generation.cost_rox),
                },
            }
...
async def create_batch(
    ...
    try:
        job, replayed = await GenerationBatchService.create(
            session,
            redis,
            user_id=user.id,
...
async def list_batches(
    ...
    jobs = list((await session.scalars(
        ...
        .order_by(BatchGenerationJob.created_at.desc())
        .limit(limit)
    )).all())
