...
class PromoProgramRequest(BaseModel):
    ...
    topup_user_rox: Decimal | None = Field(default=None, ge=0, le=100_000)
    topup_user_min_rub: Decimal | None = Field(default=None, ge=0, le=100_000_000)
    is_active: bool
...
async def web_admin_promo_program(
    context: AdminPromoReadDep,
    session: SessionDep,
) -> dict[str, object]:
...
async def web_admin_promo_program_update(
...
async def web_admin_promo_partner(
    promo_id: uuid.UUID,
