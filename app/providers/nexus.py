...
# Compatibility aliases used by the existing durable admin-test flow.
...
class NexusClient:
    ...
    async def create_nano_banana(
        ...
        *,
        ...
        if clean_model not in NANO_BANANA_MODELS:
            raise NexusProviderError(f"Unsupported Nexus Nano Banana model: {clean_model!r}")
        ...
        if len(references) > NANO_BANANA_MAX_REFERENCES:
            raise NexusProviderError(
                f"Nano Banana accepts at most {NANO_BANANA_MAX_REFERENCES} references"
            )
    ...
    async def create_nano_banana_2(
        self,
        *,
        prompt: str,
        aspect_ratio: str = "1:1",
