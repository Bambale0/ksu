...
_IMAGE_SOURCE_FIELDS = frozenset(
    {
        "image_urls",
        "input_urls",
        "image_input",
...
class GenerationActionService:
    ...
    def available_actions(cls, generation: Generation) -> list[GenerationAction]:
        ...
        # Admin trend/template prompts are deliberately hidden. They may still be
        # used as visual source material, but must never be reconstructed via a
        # repeat/new-prompt/settings flow that would reveal the saved template.
        if generation.action_type == "trend":
            if media_type == "image":
                return [item for item in source if item.id in {"remix", "edit", "animate", "publish"}]
            return [item for item in source if item.id == "publish"]
        return list(source)
    ...
    def _supports_image_source(spec: ModelSpec) -> bool:
        if spec.operation in {"motion_control", "video_edit", "video_upscale", "video_extend"}:
            return False
        return bool(set(spec.known_fields) & _IMAGE_SOURCE_FIELDS)
    ...
    def candidate_specs(cls, generation: Generation, action: str) -> list[ModelSpec]:
        ...
        if action in {"remix", "edit"}:
            return [spec for spec in specs if spec.media_type == "image" and cls._supports_image_source(spec)]
        if action == "animate":
            return [spec for spec in specs if spec.media_type == "video" and cls._supports_image_source(spec)]
        if action == "repeat":
    ...
    def default_model_id(cls, generation: Generation, action: str) -> str | None:
        ...
        if canonical in {"remix", "edit"}:
            ...
            try:
                ...
                current_family = ""
            same_family = next(
                (str(item["id"]) for item in candidates if str(item.get("family") or "") == current_family),
                None,
            )
            if same_family:
                return same_family
