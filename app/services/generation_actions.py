...
_IMAGE_EDIT_OPERATIONS = frozenset({"image_edit", "image_to_image", "generate_or_edit"})
_VIDEO_I2V_OPERATIONS = frozenset(
    ...
    def candidate_specs(cls, generation: Generation, action: str) -> list[ModelSpec]:
        ...
        if action in {"remix", "edit"}:
            return [
                ...
                and spec.operation in _IMAGE_EDIT_OPERATIONS
                and cls._supports_image_source(spec)
            ]
        if action == "animate":
            return [
                spec
                ...
                and spec.operation in _VIDEO_I2V_OPERATIONS
                and cls._supports_image_source(spec)
            ]
        if action == "repeat":
