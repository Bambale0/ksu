...
def _generic_editable_columns(table_name: str, model: type[Any]) -> list[Any]:
    ...
    for column in mapper.columns:
        if column.primary_key or column.foreign_keys:
            continue
        if column.key in {"created_at", "updated_at"} or column.key.endswith("_id"):
            continue
...
def _encode_key(model: type[Any], row: Any) -> str:
    mapper = sa_inspect(model)
...
def _decode_key(model: type[Any], encoded: str) -> list[Any]:
...
class AdminEntityService:
    @staticmethod
    def _visible_models(admin: AdminAccount) -> dict[str, type[Any]]:
        models = _models()
        if admin.role == "finance":
            return {
                name: model
                for name, model in models.items()
                if name in FINANCIAL_TABLES
            }
        return models
    ...
    def catalog(cls, *, admin: AdminAccount) -> dict[str, Any]:
        ...
        items = []
        for table_name, model in sorted(cls._visible_models(admin).items()):
            mapper = sa_inspect(model)
            editable = {column.key for column in _generic_editable_columns(table_name, model)}
            fields = [
                {
                    "name": column.key,
    ...
    def _model(cls, admin: AdminAccount, entity: str) -> type[Any]:
        model = cls._visible_models(admin).get(entity)
        if model is None:
            raise LookupError("Entity not found")
        return model

    @staticmethod
    def _view(table_name: str, model: type[Any], row: Any) -> dict[str, Any]:
        mapper = sa_inspect(model)
        editable = {column.key for column in _generic_editable_columns(table_name, model)}
        values: dict[str, Any] = {}
        for column in mapper.columns:
            values[column.key] = (
                "[redacted]"
                if _is_sensitive(column.key)
                else _json_value(getattr(row, column.key))
            )
        return {
            "record_key": _encode_key(model, row),
            "values": values,
    ...
    async def list_rows(
        ...
        *,
        ...
        if query:
            ...
            for column in mapper.columns:
                ...
                if isinstance(column.type, (String, Text)) or column.primary_key:
                    conditions.append(cast(column, String).ilike(f"%{query}%"))
            if conditions:
                predicate = or_(*conditions)
                stmt = stmt.where(predicate)
                count_stmt = count_stmt.where(predicate)

        created_column = mapper.columns.get("created_at")
        if created_column is not None:
            stmt = stmt.order_by(created_column.desc())
        else:
        ...
        return {
            "entity": entity,
            "items": [cls._view(entity, model, row) for row in rows],
    ...
    async def detail(
        ...
        *,
        ...
        AdminPolicy.require_permission(admin, "entities.read")
        model = cls._model(admin, entity)
        values = _decode_key(model, record_key)
        identity: Any = values[0] if len(values) == 1 else tuple(values)
        row = await session.get(model, identity)
        if row is None:
            raise LookupError("Entity record not found")
        return cls._view(entity, model, row)
    ...
    async def update(
        ...
        *,
        ...
        editable = {
            column.key: column
            for column in _generic_editable_columns(entity, model)
        }
        if not editable:
        ...
        normalized = {
            name: _coerce(editable[name], value)
            for name, value in changes.items()
        }

        async def operation() -> dict[str, Any]:
            mapper = sa_inspect(model)
            key_values = _decode_key(model, record_key)
            conditions = [
                column == value
                for column, value in zip(mapper.primary_key, key_values, strict=True)
            ]
            row = await session.scalar(
                select(model).where(*conditions).with_for_update()
            )
            if row is None:
                raise LookupError("Entity record not found")
