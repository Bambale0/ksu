...
## Active Feature Execution — Nano Banana Pro/2 production migration to Nexus

- **Task:** migrate customer production generation for internal models `nano_banana_pro` and `nano_banana_2` from KIE to Nexus while preserving the existing ROXY generation lifecycle, billing, history, delivery, reference handling and user-facing model identifiers.
- **Audit baseline:** `main@cde7279f36e060c1f6e6a6b5c6d58a24e35ba9ac` on 2026-09-17; branch `feat/nexus-nano-banana-pro-2`. Existing `app/providers/nexus.py` is reused and generalized rather than adding a second Nexus transport.
- **Verified Nexus contract:** both models use `POST /generate` with bearer authentication, return a `task_id`, and are polled through `GET /tasks/{task_id}`. Provider model names are `nano-banana-pro` and `nano-banana-2`; supported output sizes are `1K`, `2K`, `4K`; supported aspect ratios are `auto`, `1:1`, `16:9`, `9:16`, `4:3`, `3:4`, `3:2`, `2:3`, `5:4`, `4:5`, `21:9`; at most four image references are accepted.
- **Compatibility invariant:** all non-Nano models remain on their current provider path. Existing public model ids, pricing/ROX accounting, generation records, outbox/idempotency and media-result delivery stay unchanged. The existing Nexus admin-test flow keeps its compatibility method.
- **Contract correction:** Nano Banana Pro and Nano Banana 2 customer validation must reject Nexus-unsupported aspect ratios or more than four references before provider submission/billing can proceed; provider-specific payload translation happens only at the adapter/orchestration boundary.
- **Lifecycle:** submission chooses Nexus only for the two migrated model ids and stores `provider=nexus`; reconciliation polls Nexus for Nexus rows and KIE for KIE rows. Nexus failures/results are normalized into the existing internal generation status/result handling rather than leaking provider response shapes into domain logic.
- **No-hardcode/config:** credentials and base URL continue to come from the existing `NEXUS_API_KEY`/`NEXUS_BASE_URL` settings. The model-id→provider-model mapping is code-owned protocol metadata; prices/economics remain DB/admin-owned.
- **Verification plan:** add focused Nexus payload/model-routing/status tests first; update Nano image-contract boundary regressions; preserve existing admin Nexus and KIE tests; then run required GitHub CI/browser gates on the exact PR head. Merge only after exact-head required checks are green and review confirms no unrelated provider routing changed.
- **Rollback:** revert the isolated migration PR to restore the previous KIE routing; no schema migration or data rewrite is required.
