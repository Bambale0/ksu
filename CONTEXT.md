...
## Active Feature Execution — Nano Banana Pro / 2 → NexusAPI migration

- **Task:** migrate customer generation for `nano-banana-pro` and `nano-banana-2` from KIE to NexusAPI without changing the public model IDs, pricing contract or customer flow. `nano-banana-2-lite` stays on its existing provider.
- **Audit baseline:** `main@cde7279f36e060c1f6e6a6b5c6d58a24e35ba9ac` on 2026-09-17. Existing durable generation/outbox, refund, media localization, ReferenceResolver and provider protection paths are reused.
- **Provider contract:** NexusAPI uses `POST https://nexusapi.dev/generate` with Bearer auth and `params.model_name` (`nano-banana-pro` / `nano-banana-2`), then `GET /tasks/{task_id}` for asynchronous state. ROXY's existing `resolution` and `image_input` inputs are adapted server-side to Nexus `image_size` and `image_urls`; customer UI does not become provider-specific.
- **Acceptance criteria:** both migrated models submit with `provider=nexus`; prompt, supported aspect ratio, 1K/2K/4K quality and up to four reference images reach Nexus correctly; a Nexus `task_id` is persisted; stale Nexus generations are polled/reconciled; success enters the existing media-localization/history path; failure uses the existing idempotent refund path; KIE remains unchanged for all non-migrated models.
- **Idempotency/safety:** Nexus create calls use stable `Idempotency-Key=generation:<generation_id>`; provider identity is persisted before submission so uncertain outcomes are recovered as Nexus, not KIE; API keys remain server-only; transport/4xx/429/5xx failures are classified before retry/refund.
- **Observability:** worker protection, circuit keys, success/failure counters and reconciliation logs use the actual provider name (`nexus` or `kie`) rather than attributing Nexus failures to KIE.
- **No-hardcode:** Nexus credentials/base URL continue to come from `NEXUS_API_KEY` / `NEXUS_API_BASE_URL`; customer pricing and model visibility remain in existing catalog/config surfaces. Provider protocol constants are isolated in the Nexus adapter.
- **Verification matrix:** deterministic provider-contract tests for both model IDs and field translation; reference-limit regression; worker dispatch/recovery regression; existing Nexus admin tests; backend test/ruff/compile gates; required GitHub workflows on one exact PR SHA before merge.
- **Rollout/rollback:** branch-level change only; rollback is a revert restoring the previous KIE dispatch. No schema migration is required because `Generation.provider` already stores provider identity as text.
- **Progress:** audit and Nexus documentation verification complete; implementation branch `feat/nexus-nano-banana-migration` created; provider adapter, generation adapter, provider-aware worker/recovery and focused regression tests implemented. Exact-head CI and review are pending.
